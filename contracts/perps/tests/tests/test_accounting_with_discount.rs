...
fn accounting_with_discount_fees() {
    ...
    let atom_opening_fee_with_discount = mock.query_opening_fee("uatom", atom_size, Some(discount_pct)).fee;
    
    // Verify discount is applied correctly
    assert!(atom_opening_fee_with_discount.amount < atom_opening_fee_no_discount.amount);
    let expected_discount_amount = atom_opening_fee_no_discount.amount.checked_mul_ceil(discount_pct).unwrap();
    let expected_fee_with_discount = atom_opening_fee_no_discount.amount.checked_sub(expected_discount_amount).unwrap();
    ...
    assert!(difference <= Uint128::new(1), "Discount calculation difference too large: expected {}, got {}, difference {}", 
            expected_fee_with_discount, atom_opening_fee_with_discount.amount, difference);

    // Test different discount percentages
    let discount_25 = Decimal::percent(25);
    let atom_opening_fee_25_discount = mock.query_opening_fee("uatom", atom_size, Some(discount_25)).fee;
    ...
    let atom_opening_fee_75_discount = mock.query_opening_fee("uatom", atom_size, Some(discount_75)).fee;
    ...
    let atom_opening_fee_100_discount = mock.query_opening_fee("uatom", atom_size, Some(discount_100)).fee;
    ...
    let atom_opening_fee_0_discount = mock.query_opening_fee("uatom", atom_size, Some(discount_0)).fee;
    ...
    let small_fee_with_discount = mock.query_opening_fee("uatom", small_size, Some(discount_pct)).fee;
    
    // Verify proportional discount
    let small_expected_discount = small_fee_no_discount.amount.checked_mul_ceil(discount_pct).unwrap();
    let small_expected_fee = small_fee_no_discount.amount.checked_sub(small_expected_discount).unwrap();
    ...
    let short_fee_with_discount = mock.query_opening_fee("uatom", short_size, Some(discount_pct)).fee;
    
    assert!(short_fee_with_discount.amount < short_fee_no_discount.amount);
    let short_expected_discount = short_fee_no_discount.amount.checked_mul_ceil(discount_pct).unwrap();
    let short_expected_fee = short_fee_no_discount.amount.checked_sub(short_expected_discount).unwrap();
    ...
    let osmo_opening_fee_with_discount = mock.query_opening_fee("uosmo", osmo_size, Some(discount_pct)).fee;
    
    // Verify discount is applied consistently
    assert!(osmo_opening_fee_with_discount.amount < osmo_opening_fee_no_discount.amount);
    let osmo_expected_discount = osmo_opening_fee_no_discount.amount.checked_mul_ceil(discount_pct).unwrap();
    let osmo_expected_fee = osmo_opening_fee_no_discount.amount.checked_sub(osmo_expected_discount).unwrap();
