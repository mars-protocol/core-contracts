...
pub fn create_trigger_order(
    ...
    let contains_illegal_actions = actions
        .iter()
        .all(|action| matches!(action, Action::Deposit(..) | Action::Liquidate { .. }));
    ensure!(!contains_illegal_actions, ContractError::IllegalTriggerAction);
