...
impl StakingTierManager {
    ...
    pub fn find_applicable_tier(&self, voting_power: Uint128) -> StdResult<&FeeTier> {
        ...
        // So we find the first tier where user's voting power >= tier's min_voting_power
        ...
        // If no tier found, return the last tier (lowest threshold)
        self.config.tiers.last().ok_or_else(|| StdError::generic_err("No tiers configured"))
    }
    ...
    pub fn validate(&self) -> StdResult<()> {
        ...
        for i in 1..self.config.tiers.len() {
            let prev_power = Uint128::from_str(&self.config.tiers[i - 1].min_voting_power)
                .map_err(|_| StdError::generic_err("Invalid min_voting_power in tier"))?;
